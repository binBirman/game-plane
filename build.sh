...
# Version defaults to the workspace package version (Cargo.toml) so the tarball
# never drifts from the crate metadata. Override with VERSION=x.y.z if needed.
DEFAULT_VERSION="$(sed -n 's/^version = "\(.*\)"/\1/p' "$(dirname "${BASH_SOURCE[0]}")/Cargo.toml" | head -1)"
...
cp packaging/upgrade.sh          "$DIST/upgrade.sh"
...
chmod +x "$DIST/install.sh" "$DIST/uninstall.sh" "$DIST/upgrade.sh" "$DIST/lobby" "$DIST/tictactoe" "$DIST/take_your_position"
